...
class Philosopher implements Runnable {
    ...
    Philosopher() {
        id = instances++;
        left = DiningPhilosophers.forks.get(id);
        right = DiningPhilosophers.forks.get((id+1) % DiningPhilosophers.philosopherCount);
    }
        ...
        do {
            if (state == PhilosopherState.Pon) {    //  all that pondering
                ...
                if (token.get() == id) {            //  my turn now
                    ...
                    token.set((id+2)% DiningPhilosophers.philosopherCount);
